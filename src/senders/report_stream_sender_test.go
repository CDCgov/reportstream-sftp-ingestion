...
func (suite *SenderTestSuite) SetupTest() {
	...
	os.Setenv("CA_DPH_PRIVATE_KEY_NAME", "key")
	os.Setenv("CA_DPH_CLIENT_NAME", "client")
}

func (suite *SenderTestSuite) TearDownTest() {
	os.Unsetenv("ENV")
	os.Unsetenv("REPORT_STREAM_URL_PREFIX")
	os.Unsetenv("CA_DPH_PRIVATE_KEY_NAME")
	os.Unsetenv("CA_DPH_CLIENT_NAME")
}
...
func (suite *SenderTestSuite) Test_NewSender_VariablesAreSet_ReturnsSender() {
	...
	assert.Equal(suite.T(), os.Getenv("CA_DPH_PRIVATE_KEY_NAME"), sender.privateKeyName)
	assert.Equal(suite.T(), os.Getenv("CA_DPH_CLIENT_NAME"), sender.clientName)
}
